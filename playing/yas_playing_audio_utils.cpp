...
static uint32_t processing_length(int64_t const play_frame, int64_t const next_frame, uint32_t const file_length) {
    return std::min(uint32_t(next_frame - play_frame), uint32_t(file_length - (play_frame % file_length)));
}

static std::optional<int64_t> get_next_file_idx(int64_t const play_frame, uint32_t const proc_length,
                                                uint32_t const file_length) {
    int64_t const next_frame = play_frame + proc_length;
